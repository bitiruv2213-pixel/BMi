"""Game Arena uchun boshlang'ich ma'lumotlar (data migration)"""
from django.db import migrations


def seed_game_data(apps, schema_editor):
    TypingText = apps.get_model('courses', 'TypingText')
    CodeChallenge = apps.get_model('courses', 'CodeChallenge')
    MemoryCard = apps.get_model('courses', 'MemoryCard')

    # Agar allaqachon mavjud bo'lsa, qayta qo'shmaslik
    if TypingText.objects.exists():
        return

    # ============ TYPING TEXTS ============
    typing_data = [
        # EASY
        ("Python dasturlash tili bugungi kunda eng mashhur tillardan biri hisoblanadi. U sodda sintaksisi va kuchli kutubxonalari bilan ajralib turadi.", "easy"),
        ("Dasturlashni o'rganish sabr va amaliyot talab qiladi. Har kuni kod yozish malakangizni oshiradi va yangi imkoniyatlar eshigini ochadi.", "easy"),
        ("Kompyuter fanlari zamonaviy dunyoning asosiy yo'nalishlaridan biridir. Algoritmlar va ma'lumotlar tuzilmasi dasturlashning poydevori hisoblanadi.", "easy"),
        ("Sun'iy intellekt texnologiyalari hayotimizning barcha sohalariga kirib kelmoqda. Mashinali o'rganish va neyron tarmoqlar buning asosiy vositalaridir.", "easy"),
        ("Veb dasturlash frontend va backend qismlardan iborat. HTML sahifa tuzilmasini, CSS dizaynni, JavaScript esa interaktivlikni ta'minlaydi.", "easy"),
        # MEDIUM
        ("Django framework Python tilida yozilgan kuchli veb framework hisoblanadi. U MVC arxitekturasiga asoslangan bo'lib, tez va xavfsiz veb ilovalar yaratishga yordam beradi. Django ORM tizimi ma'lumotlar bazasi bilan ishlashni osonlashtiradi va SQL so'rovlarini avtomatik generatsiya qiladi.", "medium"),
        ("Git versiyalarni boshqarish tizimi dasturchilar uchun eng muhim vositalardan biridir. U kodning har bir o'zgarishini kuzatib boradi va jamoaviy ishlashni osonlashtiradi. Branch va merge tushunchalari parallel ravishda ishlash imkonini beradi.", "medium"),
        ("API yaratish zamonaviy dasturlashning ajralmas qismidir. REST arxitekturasi HTTP metodlari orqali ma'lumotlarni almashish imkonini beradi. JSON formati eng keng tarqalgan ma'lumot almashish formati hisoblanadi va deyarli barcha dasturlash tillari tomonidan qo'llab-quvvatlanadi.", "medium"),
        ("Ma'lumotlar bazasi har qanday dasturning asosiy qismi hisoblanadi. SQL tili relyatsion bazalar bilan ishlash uchun standart til bo'lib, SELECT, INSERT, UPDATE va DELETE buyruqlari orqali ma'lumotlarni boshqarish mumkin. Indekslar so'rovlar tezligini sezilarli oshiradi.", "medium"),
        ("Linux operatsion tizimi serverlar dunyosida eng ko'p ishlatiladigan tizimdir. Terminal orqali fayllar bilan ishlash, dasturlarni o'rnatish va tizimni boshqarish mumkin. Bash skriptlari takroriy vazifalarni avtomatlashtirish uchun ishlatiladi va DevOps sohasida keng qo'llaniladi.", "medium"),
        # HARD
        ("Mikroservislar arxitekturasi yirik dasturiy ta'minotni mustaqil xizmatlarga bo'lish yondashuvidir. Har bir mikroservis o'z ma'lumotlar bazasiga ega bo'lib, API orqali boshqa xizmatlar bilan aloqa qiladi. Docker konteynerlari va Kubernetes orkestrasiya tizimi mikroservislarni joylashtirish va boshqarishni osonlashtiradi. Bu arxitektura kengayuvchanlik va mustaqil deploy qilish imkonini beradi.", "hard"),
        ("Kriptografiya axborot xavfsizligining asosiy yo'nalishi bo'lib, ma'lumotlarni shifrlash va deshifrlash usullarini o'rganadi. Simmetrik shifrlashda bitta kalit ishlatiladi, assimetrik shifrlashda esa ochiq va yopiq kalitlar juftligi qo'llaniladi. SHA-256 va AES zamonaviy kriptografik algoritmlarning eng keng tarqalganlari hisoblanadi.", "hard"),
    ...
    for text, diff in typing_data:
        TypingText.objects.create(text=text, difficulty=diff, language='uz')
    ...
    for c in challenges:
        CodeChallenge.objects.create(**c)
...
def reverse_seed(apps, schema_editor):
    # Reverse migration - tozalash kerak emas
    pass
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('courses', '0005_memorycard'),
    ]

    operations = [
        migrations.RunPython(seed_game_data, reverse_seed),
    ]
